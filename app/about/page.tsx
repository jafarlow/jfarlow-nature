  ...
  return (
    <main className="about">
      ...
      <section className="about-content-block">
        ...
        <p>With open-source packages comes visibilty of the underlying code itself. My only recourse here was to copy in what I'll refer to as the baseline code, so that I could then force it to support basic accesibility practices. That resolved the mission critical piece. As I had pulled in the baseline code, I was then able to modify the entire structure of the code, so instead of simply being a full-screen image I built in support for image context, like captioning text. Oh, hello Cloudinary custom metadata! It's good to see you again!</p>
        <p>I made a design decision about how to display this caption. I did not want to sacrifice screen space by appending it to the bottom of the image similar to what you may see on a news article; I wanted this to hover atop the bottom of the image. I also wanted to allow the user to "dismiss" the caption and view the image without obstruction. I could achieve this by using a simple hover effect, so that when a user mouses over the image the caption appears, but this leaves out anyone who is navigating via keyboard.</p>
        <p>The first step was to make the full screen image focusable, which is not a default setting for image content. This then gave access to the keyboard for triggering the caption to appear. But I still had a bit of a UX gap here. A mouse user could "stumble upon" the caption and make it visible, thus knowing this was an option. However, that is not a reliable method for keyboard users to interact with the content. I needed a way to <i>start</i> with the image receiving focus upon going into full screen so that the caption will be there by default--no need to discover that feature.</p>
        <p>After much trial and error and lost hours googling for tips and tricks, I remembered about the cornerstone of software development: open-source code. Wait, this feels familiar.... I know what to do! I found a package called <code>focus-trap-react</code> that, in this case, <i>did</i> perfectly suit my needs. In addition to applying focus to the photo when opened into full screen, it returns the focus to whichever element had focus when the trap was activated.</p>
        {/* FOCUS TRAP!!!!! */}
